...
% xto —— the sample-scare area input data, it is 4-D format, each row is each feature, each colum is each sample
% yto —— the sample-scare area output data, it is categories
% xfrom —— the sample-abundant area input data, it is 4-D format, each row is each feature, each colum is each sample
...
Xto = reshape(xto, [size(xto,1),1,1,size(xto,2)]);
...
Xtodl = dlarray(single(Xto), 'SSCB');


Xfrom = reshape(xfrom, [size(xfrom,1),1,1,size(xfrom,2)]);
...
XGeneratedNew = minFlow+((XGeneratedNew-lb)./(ub-lb)).*(maxFlow-minFlow);
