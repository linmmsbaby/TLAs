...
ObjFcn = makeObjFcn(XTrain,YTrain,XValidation,YValidation,XX,YY,TT,AUCcnnA,CNN_OR_GRU_OR_BiLSTM);
...
function ObjFcn = makeObjFcn(XTrain,YTrain,XValidation,YValidation,XX,YY,TT,AUCcnnA,CNN_OR_GRU_OR_BiLSTM)
ObjFcn = @valErrorFun;
    function [valError,cons,fileName] = valErrorFun(optVars)
        ...
        b = predict(trainedNet,XValidation,'ExecutionEnvironment','cpu');
        b = b(:,1)';

        [X,Y,T,AUCcnn] = perfcurve(YValidation,b','landslide');
        ...
        YY = [YY;Y];
        TT = [TT;T];
        AUCcnnA = [AUCcnnA;AUCcnn];

        fileName = num2str(valError) + ".mat";
        save(fileName,'trainedNet','valError','options')
        save('file.mat','XX',"TT","YY","AUCcnnA")
        cons = [];
